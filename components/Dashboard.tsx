...
const Dashboard: React.FC = () => {
    const totalAnggota = MOCK_ANGGOTA.filter(a => a.status === 'Aktif').length;
    
    const totalSimpanan = MOCK_ANGGOTA.reduce((acc, member) => {
        return acc + member.simpanan_pokok + member.simpanan_wajib + member.simpanan_sukarela;
    }, 0);

    const outstandingMurabahah = MOCK_KONTRAK.filter(k => k.status === StatusKontrak.BERJALAN).reduce((acc, k) => {
        return acc + k.harga_jual - k.uang_muka; // Simplified calculation
    }, 0);

    const NPF = MOCK_KONTRAK.filter(k => k.status === StatusKontrak.MACET).length;

    const kpiData = [
        { title: 'Anggota Aktif', value: totalAnggota.toString(), description: 'Total anggota terdaftar' },
        { title: 'Total Simpanan', value: formatCurrency(totalSimpanan), description: 'Pokok, Wajib, & Sukarela' },
        { title: 'Outstanding Murabahah', value: formatCurrency(outstandingMurabahah), description: 'Total pembiayaan berjalan' },
        { title: 'Pembiayaan Bermasalah (NPF)', value: NPF.toString(), description: 'Kontrak dalam status macet' },
    ];
  ...
  return (
    <div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card title="Aktivitas Simpanan Terakhir">
