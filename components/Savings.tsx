...
const Savings: React.FC = () => {
    const [activeTab, setActiveTab] = useState<SavingsTab>('Data Simpanan');
    ...
    const sortedAndFilteredAnggota = useMemo(() => {
     ...
     const tableTotals = useMemo(() => {
        ...
        for (const anggota of sortedAndFilteredAnggota) {
            totals.pokok += anggota.simpanan_pokok || 0;
            totals.wajib += anggota.simpanan_wajib || 0;
            totals.sukarela += anggota.simpanan_sukarela || 0;
            totals.total += (anggota.simpanan_pokok || 0) + (anggota.simpanan_wajib || 0) + (anggota.simpanan_sukarela || 0);
        }
        return totals;
    }, [sortedAndFilteredAnggota]);
    ...
    const handleSaveTransaksi = async (transaksiData: Omit<TransaksiSimpanan, 'id' | 'anggota_id' | 'tanggal'>) => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                let newBalance = currentBalance + (transaksiData.tipe === 'Setor' ? transaksiData.jumlah : -transaksiData.jumlah);
                ...
                transaction.set(newTransaksiRef, { ...transaksiData, anggota_id: selectedAnggota.id, tanggal: new Date().toISOString() });
            });
            handleCloseModal();
        } catch (error: any) { alert(`Error: ${error.message}`); }
    };
     const handleDeleteTransaksi = async (transaksi: TransaksiSimpanan) => {
        if (!selectedAnggota || !confirm(`Yakin ingin menghapus transaksi "${transaksi.keterangan}"?`)) return;
        const memberRef = doc(db, "anggota", selectedAnggota.id);
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                if (currentBalance < 0) throw new Error("Saldo menjadi negatif!");
                ...
                <Card>
                    <div className="p-4 sm:p-6 border-b">
                        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                            ...
                            <button onClick={handleExportCsv} className="w-full md:w-auto px-4 py-2 bg-secondary text-white text-sm font-medium rounded-md hover:bg-orange-600">Export CSV</button>
                        </div>
                    </div>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                             ...
                             <tfoot className="bg-gray-100 font-bold">
                                <tr>
                                    ...
                                    <td className="px-4 py-3 text-right">{formatCurrency(tableTotals.pokok)}</td>
                                    <td className="px-4 py-3 text-right">{formatCurrency(tableTotals.wajib)}</td>
                                    <td className="px-4 py-3 text-right">{formatCurrency(tableTotals.sukarela)}</td>
                                    <td className="px-4 py-3 text-right">{formatCurrency(tableTotals.total)}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-1">
                        <Card title="Pilih Anggota">
                            <div className="p-4 border-b">
                                <input type="text" placeholder="Cari..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full p-2 border rounded-md" />
                            </div>
                    ...
                    <div className="md:col-span-2">
                        {selectedAnggota ? (
                            ...
                            <Card title="Detail Simpanan"><p className="p-6">Pilih anggota dari daftar...</p></Card>
                        )}
                    </div>
                </div>
            )}
